...
def load_item():
    ...
    if loaded is None:
        ...
        return empty, empty, empty, "0/0", "no image", refresh_thumbs()

    img, mask, high_conf, conf = loaded
    mask_rgb = mask_to_color(mask)
    return img, mask_rgb, overlay_mask(img, mask), manager.progress_text(), f"{conf:.2f} ({'high' if high_conf else 'low'})", refresh_thumbs()
...
def reset_to_auto():
    ...
    return auto_rgb, overlay_mask(img, auto_mask)
...
def refresh_thumbs():
    ...
    for pos, meta_idx in enumerate(manager.active_indices[:64]):
        ...
        _mask, high, _conf = manager.get_auto_for_meta_idx(meta_idx)
        if high:
